...
def write(title: str, theme: str, actor: str, director: str, score: str, time: int, limit_age: int, poster: str, story: str,):
    con = sqlite3.connect(db_path, isolation_level = None)
    cur = con.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS data(title str, theme str, actor str, directer str, score str, time int, limit_age int, poster str, story str)")
    
    cur.execute("SELECT * FROM data WHERE title=:title", {"title": title})
    data = cur.fetchone()
    if data is None:
        cur.execute("INSERT INTO data VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)", (title, theme, actor, director, score, time, limit_age, poster, story))
    else:
        t = ','.join(set(data[1].split(",") + [theme]))
...
for category in category_list:
    ...
    while True:
        ...
        for movie in data["result"]["productList"]:
            ...
            print((title, theme, actor, director, score, time, limit_age, poster, story))

            write(title, theme, actor, director, score, time, limit_age, poster, story)
